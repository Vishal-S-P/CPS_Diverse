...
from img_datasets.image_datasets import inverse_data_transform
import os
...
class KarrasDenoiser:
    ...
    def denoise(self, model, x_t, sigmas, t_rescale=True, **model_kwargs):
        import torch.distributed as dist
    ...
    def denoise_edm(self, model, x_t, sigmas, **model_kwargs):
        denoised = model(x_t, sigmas, **model_kwargs)
        return denoised
...
def adjusted_deterministic_DDIM(
    distiller,
    x,
    sigmas,
    generator,
    ...
    alphas_ = (1 - sigmas**2)**0.5
    t_max_rho = t_max ** (1 / rho)
    t_min_rho = t_min ** (1 / rho)
    s_in = x.new_ones([x.shape[0]])
    
    time_steps = [39/40, 30/40, 20/40, 10/40, 0/40]
    ...
    for i in range(len(time_steps)):
        ...
        eps_hat = (x - alphas_[i] * x0)/sigmas[i]
        z_s_var = (alphas_[s] - alphas_[i]* sigmas[s]/sigmas[i])**2 * (0.1/(2 +alphas_[i]**2/sigmas[i]**2))
        x = alphas_[s]*x0 + th.sqrt(sigmas[s]**2 +(256*256*3 / th.norm(eps_hat, 2).pow(2))* z_s_var)* eps_hat
        tvu.save_image(
                inverse_data_transform(x)[0], "dummy"+f"_step{ts[i]}.png"
                )
    return x
...
def stochastic_iterative_sampler(
    ...
    for i in range(len(ts) - 1):
        ...
        if plot_all:
            tvu.save_image(
                inverse_data_transform(x)[0], plot_name_base+f"_step{ts[i]}.png"
                )
    
    return x

def stochastic_optim_iterative_sampler(
    distiller,
    x,
    sigmas,
    generator,
    ...
    if c_stage:
        ...
        elif c_stage == "sampling": 
            return distiller(x, sigmas[0] * s_in)
    else:
        raise NotImplementedError("Need to pass the stage argument to use hybrid sampler")
...
def karras_sample(
    ...
    sample_fn = {
        ...
        "hybrid" : stochastic_optim_iterative_sampler,
    }[sampler]
    ...
    elif sampler == "multistep" or sampler == "hybrid":
        sampler_args = dict(
            ts=ts, t_min=sigma_min, t_max=sigma_max, rho=diffusion.rho, steps=steps, plot_all=plot_all, plot_name_base=plot_name_base,
            c_stage=c_stage, step_num=step_num
        )
    else:
        sampler_args = {}

    def denoiser(x_t, sigma):
        _, denoised = diffusion.denoise(model, x_t, sigma, t_rescale=t_rescale, **model_kwargs)
        if clip_denoised:
